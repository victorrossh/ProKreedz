...
$player=$pdo->prepare("SELECT * FROM kz_pro15 WHERE authid='".$_GET['authid']."' ORDER BY mapname ASC LIMIT 1");
$player->execute();

$records = $pdo->prepare("SELECT *, ROUND(time, 6) AS formatted_time FROM kz_pro15 WHERE authid='" . $_GET['authid'] . "' GROUP BY mapname ORDER BY mapname ASC");
$records->execute();
...
<div class="content">
    ...
    <nav>
        <ul class="menu mt-3">
            <li><a class="active" href="?page=player-pro&authid=<?php echo $_GET['authid']; ?>">Pro</a><span>(<?php echo $proRows->rowCount(); ?>)</span></li>
            <li><a href="?page=player-nub&authid=<?php echo $_GET['authid']; ?>">Nub</a><span>(<?php echo $nubRows->rowCount(); ?>)</span></li>
        </ul>
    </nav>
    ...
    <div class="box mt-5">
        <table>
            ...
            $position=$pdo->prepare("SELECT * FROM kz_pro15 WHERE mapname='".$data['mapname']."' AND time <= '".$data['time']."' ORDER BY time, name");
			$position->execute();
            ?>
            <tbody>
                <tr>
                    <td align="center"><?php echo Trophy($position->rowCount());?></td>
                    <td align="left" class="ellipsis"><a href="?page=stats-pro&map=<?php echo $data['mapname']?>"><?php echo $data['mapname']?></a></td>
